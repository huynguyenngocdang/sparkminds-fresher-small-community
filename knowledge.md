...
>
```
- query tới dấu ?. Thay vì ghi url http://example.com?page=1 thì query đại diện cho `page=1`. Ghi lại là `query: {page : 1}`
...
- prefetch: chạy khi thẻ Link xuất hiện trên viewport mà chúng ta thấy hoặc khi chúng ta scroll. Thường được sử dụng cho trang tĩnh (khi trang không cần xử lý backend) để khi nhấn vào sẽ ra ngay lập tức
